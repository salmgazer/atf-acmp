...
import { ActiveCohortCard, NoCohortCard } from "@/components/dashboard/active-cohort-card";
import { useActiveCohort, useCohortStatistics } from "@/lib/api/hooks/use-cohorts";
...
function DashboardContent() {
  ...
  const { data: activeCohort, isLoading: isLoadingActive } = useActiveCohort();
  const { data: statistics, isLoading: isLoadingStats } = useCohortStatistics(
    activeCohort?.id || ""
  );
  const { data: teamStats } = useTeamStatistics(activeCohort?.id);
  const { data: briefStats } = useBriefStatistics(activeCohort?.id);
  const { data: mentorStats } = useMentorStatistics(activeCohort?.id);
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="rounded-xl bg-card p-5 border border-border flex-1">
              ...
              <div>
                ...
                <MiniStat label="Pending Review" value={briefStatusData.pending} />
